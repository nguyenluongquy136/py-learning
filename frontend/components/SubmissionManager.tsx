...
const SubmissionManager: React.FC<Props> = ({ theme, token, onBack }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/admin/submissions?${params}`, { headers });
  ...
  useEffect(() => {
    // When serverQuery changes, reload
    load();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [serverQuery]);
  ...
  const openDetail = async (id: number) => {
    ...
    try {
      const headers: HeadersInit = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const res = await fetch(`${API_BASE_URL}/api/admin/submissions/${id}`, { headers });
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg md:text-2xl font-bold truncate">Bài nộp</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className={`relative flex items-center border rounded-lg flex-1 min-w-[240px] ${theme === 'dark' ? 'border-zinc-700 bg-zinc-900' : 'border-gray-200 bg-white'}`}>
              ...
              <input
                ...
                placeholder="Tìm theo id/user/problem (server-side)..."
              ...
              <table className="w-full min-w-[920px]">
                ...
                <tbody>
                  {filteredItems.length === 0 ? (
                    <tr>
                      <td colSpan={6} className={`py-10 px-4 text-sm ${theme === 'dark' ? 'text-zinc-400' : 'text-gray-600'}`}>
                    ...
                    <tr key={it.id} className={`border-t ${theme === 'dark' ? 'border-zinc-800 hover:bg-zinc-900/40' : 'border-gray-100 hover:bg-gray-50'}`}>
                      ...
                      <td className="py-3 px-4">
                        ...
                        <div className={`text-xs ${theme === 'dark' ? 'text-zinc-500' : 'text-gray-600'}`}>user_id: {it.user_id ?? '—'}</div>
                      </td>
                      <td className="py-3 px-4">
            ...
            <div className={`px-4 py-3 border-b flex items-center justify-between ${theme === 'dark' ? 'border-zinc-800' : 'border-gray-200'}`}>
              <div className="min-w-0">
                ...
                <div className={`text-xs ${theme === 'dark' ? 'text-zinc-400' : 'text-gray-600'} truncate`}>
                  {detail?.username ? `User: ${detail.username}` : ''}{detail?.problem_title ? ` • Problem: ${detail.problem_title}` : ''}
                </div>
              </div>
              <button
